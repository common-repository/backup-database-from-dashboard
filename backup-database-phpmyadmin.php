<?php
/*
Plugin Name: Backup Database Phpmyadmin
Description: Backup database from dashboard. 
Version: 1.0.0
Author: alexander909
...
class bd_backupDB {
	...
	function __construct() {
		...
		add_action('init', array(&$this, 'bd_init_textdomain'));
		add_action('init', array(&$this, 'bd_set_page_url'));
		add_action('admin_init', array(&$this, 'bd_update_notice_action'));
		...
		if (isset($_POST['do_backup'])) {
			...
			update_option('wp_db_backup_excs', array(
				'revisions' => $this->bd_get_revisions_to_exclude(),
				'spam' => $this->bd_get_spam_to_exclude()
			));
			switch($_POST['do_backup']) {
			case 'backup':
				add_action('init', array(&$this, 'bd_perform_backup'));
				break;
			case 'fragments':
	...
	function init() {
		...
		if (isset($_GET['backup'])) {
			...
			$this->bd_validate_file($this->backup_file);
			...
			case 'email':
				...
				if ( $success ) {
					...
					';
					echo '
						alert("' . __('Backup Complete!','backup-database') . '");
			...
			default:
				...
				if ( $success ) {
					echo '
					<script type="text/javascript">
						window.parent.setProgress("' . __('Backup Complete!','backup-database') . '");
					</script>
					';
				}
			}
		...
		if (isset($_GET['fragment'] )) {
			...
			$this->bd_validate_file($filename);
			$this->bd_backup_fragment($table, $segment, $filename);
		}

		die();
	}

	function bd_init_textdomain() {
		load_plugin_textdomain('backup-database', str_replace(ABSPATH, '', dirname(__FILE__)), dirname(plugin_basename(__FILE__)));
	}

	function bd_set_page_url() {
	 ...
	 */
	function bd_update_notice_action() {
		global $pagenow;
		if (
			(
			...
			ob_start(array(&$this, 'bd_update_notice'));
			add_action('admin_footer', create_function('', 'ob_end_flush();'));
		endif;
	}

	function bd_update_notice($text = '') {
		$pattern = '#(<a href\="' . __('http://codex.wordpress.org/WordPress_Backups') . '">.*?</p>)#';
		$replace = '$1' . "\n<p>" . sprintf(__('Click <a href="%s" target="_blank">here</a> to back up your database using the WordPress Database Backup plugin. <strong>Note:</strong> WordPress Database Backup does <em>not</em> back up your files, just your database.', 'backup-database'), 'tools.php?page=backup-database') . "</p>\n";
		$text = preg_replace($pattern, $replace, $text);
		return $text;
	}

	function bd_build_backup_script() {
		global $table_prefix, $wpdb;

		echo "<div class='wrap'>";
		echo '<fieldset class="options"><legend>' . __('Progress','backup-database') . '</legend>
			<p><strong>' .
				__('DO NOT DO THE FOLLOWING AS IT WILL CAUSE YOUR BACKUP TO FAIL:','backup-database').
			'</strong></p>
			<ol>
				<li>'.__('Close this browser','backup-database').'</li>
				<li>'.__('Reload this page','backup-database').'</li>
				<li>'.__('Click the Stop or Back buttons in your browser','backup-database').'</li>
			</ol>
			<p><strong>' . __('Progress:','backup-database') . '</strong></p>
			...
			window.onbeforeunload = function() {
				return "' . __('Navigating away from this page will cause your backup to fail.', 'backup-database') . '";
			}
		...
		case 'http':
			echo '
				setProgress("' . __('Preparing download.','backup-database') . '");
				window.onbeforeunload = null;
		...
		case 'smtp':
			...
			echo '
				setProgress("' . sprintf(__('Your backup has been emailed to %s','backup-database'), $email) . '");
				window.onbeforeunload = null;
		...
		default:
			echo '
				setProgress("' . __('Backup Complete!','backup-database') . '");
				window.onbeforeunload = null;
			';
		}
		...
		$also_backup = $this->bd_get_post_data_array('other_tables');
		$core_tables = $this->bd_get_post_data_array('core_tables');
	...
	function bd_backup_fragment($table, $segment, $filename) {
		global $table_prefix, $wpdb;

		echo "$table:$segment:$filename";

		if($table == '') {
			$msg = __('Creating backup file...','backup-database');
		} else {
			if($segment == -1) {
				$msg = sprintf(__('Finished backing up table \\"%s\\".','backup-database'), $table);
			} else {
				$msg = sprintf(__('Backing up table \\"%s\\"...','backup-database'), $table);
			}
		}
		...
		if (is_writable($this->backup_dir)) {
			...
			if(!$this->fp) {
				$this->error(__('Could not open the backup file for writing!','backup-database'));
				$this->error(array('loc' => 'frame', 'kind' => 'fatal', 'msg' =>  __('The backup file could not be saved.  Please check the permissions for writing to your backup directory and try again.','backup-database')));
			}
			else {
				if($table == '') {
					//Begin new backup of MySql
					$this->stow("# " . __('WordPress MySQL database backup','backup-database') . "\n");
					$this->stow("#\n");
					$this->stow("# " . sprintf(__('Generated: %s','backup-database'),date("l j. F Y H:i T")) . "\n");
					$this->stow("# " . sprintf(__('Hostname: %s','backup-database'),DB_HOST) . "\n");
					$this->stow("# " . sprintf(__('Database: %s','backup-database'),$this->backquote(DB_NAME)) . "\n");
					$this->stow("# --------------------------------------------------------\n");
				} else {
					if($segment == 0) {
						...
						$this->stow("# --------------------------------------------------------\n");
						$this->stow("# " . sprintf(__('Table: %s','backup-database'),$this->backquote($table)) . "\n");
						$this->stow("# --------------------------------------------------------\n");
					}
			...
			$this->error(array('kind' => 'fatal', 'loc' => 'frame', 'msg' => __('The backup directory is not writeable!  Please check the permissions for writing to your backup directory and try again.','backup-database')));
		}
			...
			addLoadEvent(function() {
				...
				for ( var k in t ) {
					...
					if ( ul ) {
						...
						if ( 2 < lis.length ) {
							...
							text.innerHTML = '<?php _e('Click and hold down <code>[SHIFT]</code> to toggle multiple checkboxes', 'backup-database'); ?>';
				...
				var initTimeChange = function() {
					...
					?> ) ) {
						...
						span.id = 'change-wrap';
						span.innerHTML = '<input type="submit" id="change-backup-time" name="change-backup-time" value="<?php _e('Change','backup-database'); ?>" />';
				...
				var clickTime = function(e, backupTime) {
					...
					backupTime.innerHTML = '<input type="text" value="' + tText + '" name="backup-time-text" id="backup-time-text" /> <span class="submit"><input type="submit" name="save-backup-time" id="save-backup-time" value="<?php _e('Save', 'backup-database'); ?>" /></span>';
	...
	function admin_load() {
		add_action('admin_head', array(&$this, 'bd_admin_header'));
	}

	function admin_menu() {
		$_page_hook = add_management_page(__('Backup','backup-database'), __('Backup','backup-database'), 'import', $this->basename, array(&$this, 'backup_menu'));
		add_action('load-' . $_page_hook, array(&$this, 'admin_load'));
		if (function_exists('get_current_screen')) {
			$screen = convert_to_screen($_page_hook);
			if (method_exists($screen,'add_help_tab')) {
				$screen->add_help_tab(array(
					'title' => __('Backup','backup-database'),
					'id' => $_page_hook,
	...
	function fragment_menu() {
		$page_hook = add_management_page(__('Backup','backup-database'), __('Backup','backup-database'), 'import', $this->basename, array(&$this, 'bd_build_backup_script'));
		add_action('load-' . $page_hook, array(&$this, 'admin_load'));
	}

	/**
	 * Add backup-database-specific help options to the 2.7 =< WP contextual help menu
	 * @return string The text of the help menu.
	 */
	function help_menu() {
		$text = "\n<a href=\"http://wordpress.org/extend/plugins/backup-database/faq/\" target=\"_blank\">" . __('FAQ', 'backup-database') . '</a>';
		return $text;
	}
	...
	function stow($query_line) {
		if(false === @fwrite($this->fp, $query_line))
			$this->error(__('There was an error writing a line to the backup script:','backup-database') . '  ' . $query_line . '  ' . $php_errormsg);
	}
	...
	function error_display($loc = 'main', $echo = true) {
		...
		if ( 10 == count( $err_list ) )
			$err_list[9] = __('Subsequent errors have been omitted from this log.','backup-database');
	...
	function backup_table($table, $segment = 'none') {
		...
		if (! $table_structure) {
			$this->error(__('Error getting table details','backup-database') . ": $table");
			return false;
		}
		...
		if(($segment == 'none') || ($segment == 0)) {
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('Delete any existing table %s','backup-database'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
			$this->stow("\n");
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('Table structure of table %s','backup-database'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
			$this->stow("\n");

			$create_table = $wpdb->get_results("SHOW CREATE TABLE $table", ARRAY_N);
			if (false === $create_table) {
				$err_msg = sprintf(__('Error with SHOW CREATE TABLE for %s.','backup-database'), $table);
				$this->error($err_msg);
				$this->stow("#\n# $err_msg\n#\n");
			}
			...
			if (false === $table_structure) {
				$err_msg = sprintf(__('Error getting table structure of %s','backup-database'), $table);
				$this->error($err_msg);
				$this->stow("#\n# $err_msg\n#\n");
			}
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow('# ' . sprintf(__('Data contents of table %s','backup-database'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
		}
		...
		if(($segment == 'none') || ($segment < 0)) {
			...
			$this->stow("\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('End of data contents of table %s','backup-database'),$this->backquote($table)) . "\n");
			$this->stow("# --------------------------------------------------------\n");
			$this->stow("\n");
		}
	} // end backup_table()
	...
	function db_backup($core_tables, $other_tables) {
		...
		if (is_writable($this->backup_dir)) {
			...
			$this->error(__('The backup directory is not writeable!','backup-database'));
			return false;
		}
		...
		if ('http' == $delivery) {
			if ( ! file_exists( $diskfile ) ) {
				if ( empty( $_GET['download-retry'] ) ) {
					$this->error(array('kind' => 'fatal', 'msg' => sprintf(__('File not found:%s','backup-database'), "&nbsp;<strong>$filename</strong><br />") . '<br /><a href="' . $this->page_url . '">' . __('Return to Backup','backup-database') . '</a>'));
				} else {
					return true;
				}
			...
			if (! file_exists($diskfile)) {
				$msg = sprintf(__('File %s does not exist!','backup-database'), $diskfile);
				$this->error($msg);
				return false;
			}
			...
			$message = sprintf(__("Attached to this email is\n   %1s\n   Size:%2s kilobytes\n",'backup-database'), $filename, round(filesize($diskfile)/1024));
			$success = $this->send_mail($recipient, get_bloginfo('name') . ' ' . __('Database Backup','backup-database'), $message, $diskfile);

			if ( false === $success ) {
				$msg = __('The following errors were reported:','backup-database') . "\n ";
				if ( function_exists('error_get_last') ) {
					$err = error_get_last();
					$msg .= $err['message'];
				} else {
					$msg .= __('ERROR: The mail application has failed to deliver the backup.','backup-database');
				}
	...
	function backup_menu() {
		...
		if ( $this->backup_complete ) {
			$feedback = '<div class="updated backup-database-updated"><p>' . __('Backup Successful','backup-database') . '!';
			$file = $this->backup_file;
			switch($_POST['deliver']) {
			case 'http':
				$feedback .= '<br />' . sprintf(__('Your backup file: %2s should begin downloading shortly.','backup-database'), "{$this->backup_file}", $this->backup_file);
				break;
			case 'smtp':
				$email = sanitize_text_field(wp_unslash($_POST['backup_recipient']));
				if (! is_email($email)) {
				...
				$feedback = '<br />' . sprintf(__('Your backup has been emailed to %s','backup-database'), $feedback);
				break;
			}
			$feedback .= '</p></div>';
		}
		...
		if (count($this->errors)) {
			$feedback .= '<div class="updated backup-database-updated error"><p><strong>' . __('The following errors were reported:','backup-database') . '</strong></p>';
		...
		if ( (function_exists('wp_schedule_event') || function_exists('wp_cron_init'))
			...
			$feedback .= '<div class="updated backup-database-updated"><p>' . __('Scheduled Backup Options Saved!','backup-database') . '</p></div>';
		endif;
...
//		$dir_perms = $stat['mode'] & 0000777; // Get the permission bits.
		...
		if ( ! file_exists($this->backup_dir) && ! @mkdir($this->backup_dir) ) {
			?><div class="updated backup-database-updated error"><p><?php _e('WARNING: Your backup directory does <strong>NOT</strong> exist, and we cannot create it.','backup-database'); ?></p>
			<p><?php printf(__('Using your FTP client, try to create the backup directory yourself: %s', 'backup-database'), '<code>' . $this->backup_dir . '</code>'); ?></p></div><?php
			$whoops = true;
		// not writable due to write permissions
		} elseif ( !is_writable($this->backup_dir) && ! @chmod($this->backup_dir, $dir_perms) ) {
			?><div class="updated backup-database-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable! We cannot create the backup files.','backup-database'); ?></p>
			<p><?php printf(__('Using your FTP client, try to set the backup directory&rsquo;s write permission to %1$s or %2$s: %3$s', 'backup-database'), '<code>777</code>', '<code>a+w</code>', '<code>' . $this->backup_dir . '</code>'); ?>
			</p></div><?php
			$whoops = true;
		} else {
			...
			// the directory is not writable probably due to safe mode
				...
				?><div class="updated backup-database-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable! We cannot create the backup files.','backup-database'); ?></p><?php
				if( ini_get('safe_mode') ){
					?><p><?php _e('This problem seems to be caused by your server&rsquo;s <code>safe_mode</code> file ownership restrictions, which limit what files web applications like WordPress can create.', 'backup-database'); ?></p><?php
				}
				?><?php printf(__('You can try to correct this problem by using your FTP client to delete and then re-create the backup directory: %s', 'backup-database'), '<code>' . $this->backup_dir . '</code>');
				?></div><?php
				$whoops = true;
			}
		}
		...
		?><div class='wrap'>
		<h2><?php _e('Backup','backup-database') ?></h2>
		<form method="post" action="">
		<?php if ( function_exists('wp_nonce_field') ) wp_nonce_field($this->referer_check_key); ?>
		<fieldset class="options"><legend><?php _e('Tables','backup-database') ?></legend>
		<div class="tables-list core-tables alternate">
		<h4><?php _e('These core WordPress tables will always be backed up:','backup-database') ?></h4><ul><?php
		$excs = (array) get_option('wp_db_backup_excs');
		foreach ($wp_backup_default_tables as $table) {
			if ( $table == $wpdb->comments ) {
				...
				echo "<li><input type='hidden' name='core_tables[]' value='$table' /><code>$table</code> <span class='instructions'> <input type='checkbox' name='exclude-spam[]' value='$table' $checked /> " . __('Exclude spam comments', 'backup-database') . '</span></li>';
				...
				echo "<li><input type='hidden' name='core_tables[]' value='$table' /><code>$table</code> <span class='instructions'> <input type='checkbox' name='exclude-revisions[]' value='$table' $checked /> " . __('Exclude post revisions', 'backup-database') . '</span></li>';
			} else {
		...
		if (count($other_tables) > 0) {
			?>
			<h4><?php _e('You may choose to include any of the following tables:','backup-database'); ?></h4>
			<ul>
			<?php
		...
		<fieldset class="options">
			...
			<p class="submit">
				<input type="submit" name="submit" onclick="document.getElementById('do_backup').value='fragments';" value="<?php _e('Backup now!','backup-database'); ?>" />
			</p>
			<?php else : ?>
				<div class="updated backup-database-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable!','backup-database'); ?></p></div>
			<?php endif; // ! whoops ?>
		</fieldset>
		...
		if ( $cron_old || $cron ) :
			echo '<fieldset class="options"><legend>' . __('Scheduled Backup','backup-database') . '</legend>';
			...
			if ( $cron ) :
				...
				if ( ! empty( $next_cron ) ) :
					...
					<?php printf(__('Next Backup: %s','backup-database'), '<span id="next-backup-time">' . gmdate($datetime, $next_cron + (get_option('gmt_offset') * 3600)) . '</span>'); ?>
					</p>
					<?php
				endif;
			elseif ( $cron_old ) :
				?><p><?php printf(__('Last WP-Cron Daily Execution: %s','backup-database'), gmdate($datetime, get_option('wp_cron_daily_lastrun') + (get_option('gmt_offset') * 3600))); ?><br /><?php
				printf(__('Next WP-Cron Daily Execution: %s','backup-database'), gmdate($datetime, (get_option('wp_cron_daily_lastrun') + (get_option('gmt_offset') * 3600) + 86400))); ?></p><?php
			endif;
			?><form method="post" action="">
			<?php if ( function_exists('wp_nonce_field') ) wp_nonce_field($this->referer_check_key); ?>
			<div class="tables-list">
			<h4><?php _e('Schedule: ','backup-database'); ?></h4>
			<?php
			if ( $cron_old ) :
				$wp_cron_backup_schedule = get_option('wp_cron_backup_schedule');
				$schedule = array(0 => __('None','backup-database'), 1 => __('Daily','backup-database'));
			...
			$cron_recipient_input = '<p><label for="cron_backup_recipient">' . __('Email backup to:','backup-database') . ' <input type="text" name="cron_backup_recipient" id="cron_backup_recipient" size="20" value="' . $cron_recipient . '" /></label></p>';
			echo apply_filters('wp_db_b_cron_recipient_input', $cron_recipient_input);
			echo '<p class="submit"><input type="submit" name="submit" value="' . __('Schedule backup','backup-database') . '" /></p>';
			echo '</div>';
			...
			if (count($other_tables) > 0) {
				...
				echo '<h4>' . __('Tables to include in the scheduled backup:','backup-database') . '</h4><ul>';
				foreach ($other_tables as $table) {
	...
	function schedule_choices($schedule) { // create the cron menu based on the schedule
		...
		$menu = '<ul>';
		$schedule = array_merge( array( 'never' => array( 'interval' => 0, 'display' => __('Never','backup-database') ) ),
			(array) $schedule_sorted );
		foreach ( $schedule as $name => $settings) {
			$interval = (int) $settings['interval'];
			if ( 0 == $interval && ! 'never' == $name ) continue;
			$display = ( ! '' == $settings['display'] ) ? $settings['display'] : sprintf(__('%s seconds','backup-database'),$interval);
	...
	function add_sched_options($sched) {
		$sched['weekly'] = array('interval' => 604800, 'display' => __('Once Weekly','backup-database'));
		return $sched;
	}
	...
	function can_user_backup($loc = 'main') {
		...
		if ( false == $can )
			$this->error(array('loc' => $loc, 'kind' => 'fatal', 'msg' => __('You are not allowed to perform backups.','backup-database')));
		return $can;
	}
	...
	function verify_nonce($rec = '', $nonce = 'X', $loc = 'main') {
		if ( wp_verify_nonce($rec, $nonce) )
			...
			$this->error(array('loc' => $loc, 'kind' => 'fatal', 'msg' => sprintf(__('There appears to be an unauthorized attempt from this site to access your database located at %1s.  The attempt has been halted.','backup-database'),get_option('home'))));
	}
	 ...
	 * @return null
	 */
	function bd_validate_file($file) {
		if ( (false !== strpos($file, '..')) || (false !== strpos($file, './')) || (':' == substr($file, 1, 1)) )
			$this->error(array('kind' => 'fatal', 'loc' => 'frame', 'msg' => __("Cheatin' uh ?",'backup-database')));
	}
	 ...
	 * @return array
	 */
	function bd_sanitize_array($array_to_sanitize) {
		$sanitized = array();
	 ...
	 * @return array
	 */
	function bd_get_post_data_array($post_key) {
		$sanitized_data = array();
		if (isset( $_POST[$post_key] )) {
			$sanitized_data = (array) $_POST[$post_key];
		}
		return $this->bd_sanitize_array($sanitized_data);
	}
	 ...
	 * @return array
	 */
	function bd_get_revisions_to_exclude() {
		return $this->bd_get_post_data_array('exclude-revisions');
	}
	 ...
	 * @return array
	 */
	function bd_get_spam_to_exclude() {
		return $this->bd_get_post_data_array('exclude-spam');
	}
	...
	function get_submitted_tables_to_backup_in_cron() {
		return $this->bd_get_post_data_array('wp_cron_backup_tables');
	}
}

function bd_wpdbBackup_init() {
	global $mywpdbbackup;
	$mywpdbbackup = new bd_backupDB();
}

add_action('plugins_loaded', 'bd_wpdbBackup_init');
?>
